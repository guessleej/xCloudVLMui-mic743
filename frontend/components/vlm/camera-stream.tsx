...
export default function CameraStream({
  ...
  const stopCamera = useCallback(() => {
    ...
    // 深遠鏡頭：清空 MJPEG src 停止串流
    if (serverImgRef.current) serverImgRef.current.src = "";
    setIsCameraOn(false);
    setVideoSize({ w: 0, h: 0 });
  }, []);

  // 啟動深遠鏡頭（伺服器端 MJPEG）
  const startServerCamera = useCallback(() => {
    stopCamera();
    if (!serverImgRef.current) return;
    const mjpegUrl = "/vlm/stream";   // nginx 已代理 → 同源 HTTPS
    serverImgRef.current.src = mjpegUrl;
    serverImgRef.current.onload = () => setIsCameraOn(true);
    serverImgRef.current.onerror = () => {
      toast.error("無法連線至深遠鏡頭，請確認 vlm-webui 服務正常運作。");
    };
    // MJPEG 是持續串流，img onload 只觸發第一幀，直接設為開啟
    setIsCameraOn(true);
